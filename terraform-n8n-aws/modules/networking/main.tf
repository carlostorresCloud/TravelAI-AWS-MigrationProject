...
# Public subnets only -- no NAT Gateway, since NAT Gateways are NOT Free Tier eligible.
...
# Security group for RDS -- only reachable from the EC2 security group, not the internet
resource "aws_security_group" "rds" {
  name        = "${var.project_name}-rds-sg"
  description = "Allow Postgres access only from the n8n EC2 instance"
  vpc_id      = aws_vpc.main.id
